...
class TA_TRIX (Indicator):
    ...
    def calculate(self, candles):        
        ...
        if candles_len < self.period*3:
            return Decimal(0)
        
        val_array = np.array(map(lambda x: float(x['ohlc'].close), candles[-self.period*3:]))
        
        #calculate trix
        cur_trix = talib.TRIX (val_array, timeperiod=self.period)
        
        log.info ("TA_TRIX: %s"%str(cur_trix))
